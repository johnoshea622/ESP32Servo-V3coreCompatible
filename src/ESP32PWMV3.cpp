...
double ESP32PWM::_ledcSetupTimerFreq(uint8_t channel, double freq, uint8_t bit_num) {
...
int ESP32PWM::allocatenext(double freq) {
    ...
    ESP_LOGE(TAG, "ERROR All PWM timers allocated! Can't accommodate %d Hz\r\nHalting...", freq);
    while (1);
}
...
void ESP32PWM::deallocate() {
    ...
    if (timerCount[getTimer()] == 0) {
        timerFreqSet[getTimer()] = -1; // last pwm closed out
    }
    timerNum = -1;
    attachedState = false;
...
double ESP32PWM::setup(double freq, uint8_t resolution_bits) {
    ...
    if (attached()) {
        ledcDetachPin(pin);
        double val = ledcSetup(pwmChannel, freq, resolution_bits);
...
void ESP32PWM::adjustFrequencyLocal(double freq, double dutyScaled) {
    ...
    if (attached()) {
        ...
        _ledcSetupTimerFreq(pin, freq, resolutionBits);
        writeScaled(dutyScaled);
        ledcAttachPin(pin, pwmChannel); // re-attach the pin after frequency adjust
    } else {
        _ledcSetupTimerFreq(pin, freq, resolutionBits);
        writeScaled(dutyScaled);
    }
    ...
    for (int i = 0; i < timerCount[getTimer()]; i++) {
        ...
        if (ChannelUsed[pwm] != NULL) {
            if (ChannelUsed[pwm]->getTimer() == getTimer()) {
                ...
                if (abs(diff) > 0.1) {
                    ESP_LOGW(TAG,
                             "WARNING: PWM channel %d shares a timer with channel %d. Changing the frequency to %d Hz will also change channel %d from its previous frequency of %d Hz.",
